    ...
    public function loadToolConsumer($consumer) {
        ...
        if (!empty($consumer->getRecordId())) {
            ...
            $sql = sprintf('SELECT consumer_pk, name, consumer_key256, consumer_key, secret, lti_version, ' .
                    ...
                    'enable_from, enable_until, last_access, created, updated ' .
                    "FROM {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' ' .
                    "WHERE consumer_key256 = %s", DataConnector::quoted($key256));
        }
    ...
    public function saveToolConsumer($consumer) {
        ...
        if (empty($id)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' (consumer_key256, consumer_key, name, ' .
                    ...
                    'enable_from, enable_until, last_access, created, updated) ' .
                    'VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %d, %s, %s, %s, %s, %s)', DataConnector::quoted($key256), DataConnector::quoted($key), DataConnector::quoted($consumer->name), DataConnector::quoted($consumer->secret), DataConnector::quoted($consumer->ltiVersion), DataConnector::quoted($consumer->consumerName), DataConnector::quoted($consumer->consumerVersion), DataConnector::quoted($consumer->consumerGuid), DataConnector::quoted($profile), DataConnector::quoted($consumer->toolProxy), DataConnector::quoted($settingsValue), $protected, $enabled, DataConnector::quoted($from), DataConnector::quoted($until), DataConnector::quoted($last), DataConnector::quoted($now), DataConnector::quoted($now));
        } else {
...
###
    ...
    public function loadContext($context) {
        ...
        if (!empty($context->getRecordId())) {
            ...
            $sql = sprintf('SELECT context_pk, consumer_pk, lti_context_id, settings, created, updated ' .
                    "FROM {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' ' .
                    'WHERE (consumer_pk = %d) AND (lti_context_id = %s)', $context->getConsumer()->getRecordId(), DataConnector::quoted($context->ltiContextId));
        }
    ...
    public function saveContext($context) {
        ...
        if (empty($id)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' (consumer_pk, lti_context_id, ' .
                    ...
                    'VALUES (%d, %s, %s, %s, %s)', $consumer_pk, DataConnector::quoted($context->ltiContextId), DataConnector::quoted($settingsValue), DataConnector::quoted($now), DataConnector::quoted($now));
        } else {
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' SET ' .
                    'lti_context_id = %s, settings = %s, ' .
                    'updated = %s' .
                    'WHERE (consumer_pk = %d) AND (context_pk = %d)', DataConnector::quoted($context->ltiContextId), DataConnector::quoted($settingsValue), DataConnector::quoted($now), $consumer_pk, $id);
        }
        $ok = mysqli_query($this->db, $sql);
        if ($ok) {
            if (empty($id)) {
...
// Delete context
        $sql = sprintf('DELETE c ' .
                "FROM {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' c ', 'WHERE c.context_pk = %d', $context->getRecordId());
        $ok = mysqli_query($this->db, $sql);
        if ($ok) {
            $context->initialize();
        }
...
// Delete resource link
    ...
    public function getUserResultSourcedIDsResourceLink($resourceLink, $localOnly, $idScope) {
        ...
        if ($localOnly) {
            ...
            $sql = sprintf('SELECT u.user_pk, u.lti_result_sourcedid, u.lti_user_id, u.created, u.updated ' .
                    ...
                    '((rl.primary_resource_link_pk = %d) AND (share_approved = 1))', $resourceLink->getRecordId(), $resourceLink->getRecordId());
        }
    ...
    public function getSharesResourceLink($resourceLink) {
        ...
        $sql = sprintf('SELECT consumer_pk, resource_link_pk, share_approved ' .
                ...
                'ORDER BY consumer_pk', $resourceLink->getRecordId());
...
// Load the nonce
        $sql = sprintf("SELECT value AS T FROM {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . ' WHERE (consumer_pk = %d) AND (value = %s)', $nonce->getConsumer()->getRecordId(), DataConnector::quoted($nonce->getValue()));
        $rs_nonce = mysqli_query($this->db, $sql);
        if ($rs_nonce) {
            $row = mysqli_fetch_object($rs_nonce);
            if ($row === null) {
                $ok = false;
            }
        }
...
// Load share key
        $id = mysqli_real_escape_string($shareKey->getId());
    ...
    public function saveResourceLinkShareKey($shareKey) {
        ...
        $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_SHARE_KEY_TABLE_NAME . ' ' .
                ...
                "VALUES (%s, %d, {$approve}, '{$expires}')", DataConnector::quoted($shareKey->getId()), $shareKey->resourceLinkId);
        $ok = mysqli_query($this->db, $sql);
    ...
    public function loadUser($user) {
        ...
        if (!empty($user->getRecordId())) {
            $sql = sprintf('SELECT user_pk, resource_link_pk, lti_user_id, lti_result_sourcedid, created, updated ' .
                    ...
                    'WHERE (user_pk = %d)', $user->getRecordId());
        } else {
            $sql = sprintf('SELECT user_pk, resource_link_pk, lti_user_id, lti_result_sourcedid, created, updated ' .
                    "FROM {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' ' .
                    'WHERE (resource_link_pk = %d) AND (lti_user_id = %s)', $user->getResourceLink()->getRecordId(), DataConnector::quoted($user->getId(ToolProvider\ToolProvider::ID_SCOPE_ID_ONLY)));
        }
        $rsUser = mysqli_query($this->db, $sql);
        if ($rsUser) {
    ...
    public function saveUser($user) {
        ...
        if (is_null($user->created)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' (resource_link_pk, ' .
                    ...
                    'VALUES (%d, %s, %s, %s, %s)', $user->getResourceLink()->getRecordId(), DataConnector::quoted($user->getId(ToolProvider\ToolProvider::ID_SCOPE_ID_ONLY)), DataConnector::quoted($user->ltiResultSourcedId), DataConnector::quoted($now), DataConnector::quoted($now));
        } else {
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' ' .
                    'SET lti_result_sourcedid = %s, updated = %s ' .
                    'WHERE (user_pk = %d)', DataConnector::quoted($user->ltiResultSourcedId), DataConnector::quoted($now), $user->getRecordId());
        }
        $ok = mysqli_query($this->db, $sql);
        if ($ok) {
            if (is_null($user->created)) {
                $user->setRecordId(mysqli_insert_id($this->db));
                $user->created = $time;
            }
            $user->updated = $time;
        }
